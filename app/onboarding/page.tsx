...
export default function OnboardingPage() {
  const router = useRouter(); const { store, setUserName, setPurpose, addGoal } = useGoatStore()
  ...
  return <main className="onboarding-page"><header><div className="brand-mark"><span>G</span><strong>GOATS</strong></div><div className="onboarding-progress"><small>ETAPA {step} DE 3</small><div>{[1,2,3].map(item=><span key={item} className={item<=step?'active':''}/>)}</div></div></header><section className="onboarding-card">
    ...
    <footer>{step>1?<button className="onboarding-back" onClick={()=>setStep(step-1)}><ArrowLeft size={17}/> Voltar</button>:<span/>}<button className="onboarding-next" disabled={!canContinue||loading} onClick={()=>step<3?setStep(step+1):void finish()}>{loading?'Preparando seu sistema...':step<3?'Continuar':'Entrar no GOATS'}<ArrowRight size={18}/></button></footer>
  </section></main>
}
