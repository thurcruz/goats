...
export default function TopBar() {
  ...
  return <header className="glass sticky top-0 z-40 flex h-14 items-center justify-between px-4 md:hidden">
    <Link href="/dashboard" className="flex items-center gap-2 text-white no-underline">
      ...
      <strong className="text-sm tracking-[.18em]">GOATS</strong>
    </Link>
    <div className="flex items-center gap-2">
      <Link href="/goat-ai" aria-label="Cabra (IA)" className="grid h-9 w-9 place-items-center rounded-full border" style={{ borderColor: active('/goat-ai') ? 'rgba(208,224,39,.45)' : 'var(--line)', color: active('/goat-ai') ? 'var(--energy)' : 'var(--ink)' }}><Bot size={18}/></Link>
