...
export default function TopBar() {
  ...
  return <header className="glass sticky top-0 z-40 flex h-14 items-center justify-between px-4 md:hidden">
    ...
    <div className="flex items-center gap-2">
      <Link href="/apri" aria-label="Apri" className="grid h-9 w-9 place-items-center rounded-full border" style={{ borderColor: active('/apri') ? 'rgba(208,224,39,.45)' : 'var(--line)', color: active('/apri') ? 'var(--energy)' : 'var(--ink)' }}><Bot size={18}/></Link>
