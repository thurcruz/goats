...
const system = [
  { href: '/goat-ai', label: 'Cabra (IA)', icon: Bot },
...
export default function Sidebar() {
  ...
  const initials = name.split(' ').map(part => part[0]).join('').slice(0, 2).toUpperCase() || 'G'
  ...
  return <aside className="glass fixed bottom-5 left-5 top-5 z-40 hidden w-[224px] flex-col overflow-hidden rounded-[28px] p-4 md:flex">
    <Link href="/dashboard" className="mb-4 flex shrink-0 items-center gap-3 px-2 text-white no-underline"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-energy text-[#11130f]"><Sparkles size={20}/></span><div><strong className="tracking-[.16em]">GOATS</strong><p className="muted text-[10px]">EVOLUA TODO DIA</p></div></Link>
